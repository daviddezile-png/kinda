...
import { StudentPicker, type PickClass } from "@/components/student/StudentPicker"
  ...
  const data: PickClass[] = [
    {
      id: klass.id,
      name: klass.name,
      students: klass.students.map((s) => ({ id: s.id, name: s.name, avatar: s.avatar })),
    },
  ]
    ...
    <div className="relative min-h-screen overflow-hidden px-4 py-8">
      ...
      <div className="relative z-10 mx-auto max-w-4xl">
        ...
        <div className="mb-4 flex flex-wrap justify-end gap-2">
          <Link href="/student/join" className="btn-press glass-card rounded-full px-4 py-2 text-sm font-bold text-gray-500">
            Change class
          </Link>
          <Link href="/teacher" className="btn-press glass-card rounded-full px-4 py-2 text-sm font-bold text-gray-500">
            Teacher login
          </Link>
        </div>
        <h1 className="mb-6 text-center text-3xl font-black text-gray-700">Who is learning today?</h1>
