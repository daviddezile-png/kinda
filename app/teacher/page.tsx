...
export default async function TeacherDashboard() {
  ...
  const studentIds =
    klass?.students.map((student: { id: string }) => student.id) ?? [];
  ...
  const levelCounts = ALL_STEPS.map(
    (lvl) =>
      klass?.students.filter(
        (student: { level: number }) => clampLevel(student.level) === lvl,
      ).length ?? 0,
  );
          ...
          <div className="grid gap-5 md:grid-cols-3">
            <div className="rounded-3xl bg-white/80 p-5 shadow-lg md:col-span-1">
              ...
              {klass.code && (
                <div className="mt-4 border-t border-gray-100 pt-3">
                  ...
                  <p className="font-mono text-2xl font-black tracking-widest text-indigo-600">
                    {klass.code}
                  </p>
                  <p className="text-xs text-gray-400">
                    Students enter this at /student/join
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="rounded-3xl bg-white/80 p-5 shadow-lg md:col-span-2">
              ...
              <p className="text-sm text-gray-500">
                <span className="font-bold">{curriculum?.letters.length}</span>{" "}
                letters · {curriculum?.sequential ? "in order" : "any order"}
              </p>
              <div className="mt-2 flex flex-wrap gap-1.5">
                {curriculum?.letters.map((l) => (
                  <span
                    key={l}
                    className="grid h-8 w-8 place-items-center rounded-lg bg-gray-100 text-sm font-black text-gray-600"
                  >
                    {l}
                  </span>
                ))}
                    ...
                    >
                      ...
                      <div className="flex h-20 w-full items-end rounded-lg bg-gray-50">
                        <div
                          ...
                          style={{
                            height: `${(levelCounts[idx] / maxLevelCount) * 100}%`,
                          }}
                        />
                      </div>
            ...
            <div className="rounded-3xl bg-white/80 p-5 shadow-lg md:col-span-3">
              ...
              {analytics.totalStudents > 0 && (
                <p className="mt-3 text-xs text-gray-400">
                  ...
                  · {analytics.started}/{analytics.totalStudents} children
                  started
                </p>
              )}
            </div>
              ...
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4">
                {klass.students.map(
                  (student: {
                    ...
                    const sup = analytics.byStudent.get(student.id);
                    const dot =
                      sup?.level === "support"
                        ? "bg-amber-400"
                        : sup?.level === "good"
                    ...
                    return (
                      ...
                      >
                        <span className="relative shrink-0">
                          ...
                          <span
                            title={
                              sup?.needsSupport ? "Needs support" : "On track"
                            }
