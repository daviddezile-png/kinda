...
export default async function StudentAnalyticsPage({ params }: PageProps) {
  ...
  if (!student) notFound()
  if (session.user.role === "TEACHER" && student.class.teacherId !== session.user.id) redirect("/teacher")
  ...
  const progress = await prisma.progress.findMany({ where: { studentId: id, module: "LETTERS" } })
  const rewards = await prisma.studentReward.count({ where: { studentId: id } })

  // Score this child against classmates so "falling behind" is meaningful.
  const analytics = await getClassAnalytics(student.class.students.map((s) => s.id))
  const insight = analytics.byStudent.get(id)

  // index by "level:letter"
  const cell = new Map(progress.map((p) => [`${p.step}:${p.itemId}`, p]))
  const totalStars = progress.reduce((n, p) => n + p.stars, 0)
  const totalAttempts = progress.reduce((n, p) => n + p.attempts, 0)
  const completedCount = progress.filter((p) => p.completed).length
  const lastActive = progress.reduce<Date | null>(
    (latest, p) => (p.completedAt && (!latest || p.completedAt > latest) ? p.completedAt : latest),
    null,
  )
      ...
      <div className="relative z-10 mx-auto max-w-4xl">
        <header className="mb-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Picture src={ch?.image} alt={ch?.label ?? student.name} emoji={ch?.emoji} emojiClassName="text-3xl" className="h-12 w-12 object-contain" />
            <div>
              <h1 className="text-2xl font-black text-gray-700">{student.name}</h1>
              <p className="text-sm text-gray-400">
                {student.class.name} · on <span className="font-bold text-indigo-500">Lv {level} · {STEP_LABELS[level]}</span>
              </p>
            </div>
          </div>
          <Link href="/teacher/students" className="rounded-full bg-white/70 px-4 py-2 text-sm font-bold text-gray-500 shadow">
            ← Students
          </Link>
        </header>
              ...
              <>
                ...
                <ul className="mt-2 space-y-1">
                  {insight.reasons.map((r) => (
                    <li key={r} className="text-sm font-bold text-amber-600">• {r}</li>
                  ))}
                </ul>
                <p className="mt-2 text-xs text-amber-500/80">
                  {insight.completed} stages done vs class average {analytics.avgCompleted.toFixed(1)}
                  {insight.retries > 0 ? ` · ${insight.retries} repeats` : ""}
                </p>
              </>
            ) : insight.completed === 0 ? (
              <p className="font-black text-gray-500">Hasn&apos;t started yet.</p>
            ) : (
              <p className="font-black text-green-600">
                Doing well — {insight.completed} stages done
                {insight.completed >= analytics.avgCompleted ? ", at or above the class average." : "."}
              </p>
            )}
          </div>
        )}
        ...
        <div className="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
          ...
          <Stat label="Last active" value={lastActive ? lastActive.toLocaleDateString() : "—"} small />
        </div>
        ...
        <div className="overflow-x-auto rounded-3xl bg-white/80 p-5 shadow-lg">
          <table className="w-full border-separate border-spacing-1 text-center">
            <thead>
              <tr>
                <th className="px-2 py-1 text-left text-xs font-bold uppercase tracking-wide text-gray-400">Stage</th>
                {letters.map((l) => (
                  <th key={l} className="px-1 text-sm font-black text-gray-500">{l}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {ALL_STEPS.map((lvl) => (
                <tr key={lvl}>
                  <td className={`whitespace-nowrap px-2 py-1 text-left text-xs font-bold ${lvl === level ? "text-indigo-600" : "text-gray-500"}`}>
                    {lvl} · {STEP_LABELS[lvl]}
                  </td>
                  {letters.map((l) => {
                    const p = cell.get(`${lvl}:${l}`)
                      ...
                      <td key={l} className="px-0.5">
                        <span
                          ...
                          className={`grid h-8 w-8 place-items-center rounded-lg text-sm ${
                            done ? "bg-green-100 text-green-600" : p ? "bg-amber-50 text-amber-500" : "bg-gray-50 text-gray-300"
                          }`}
                        >
                          {done ? "★" : p ? "·" : ""}
                        </span>
          ...
          <p className="mt-3 text-xs text-gray-400">
            ★ completed · · started · {totalAttempts} total attempts across all stages
          </p>
        </div>
      </div>
    </div>
...
function Stat({ label, value, accent, small }: { label: string; value: number | string; accent?: string; small?: boolean }) {
  return (
    <div className="rounded-3xl bg-white/80 p-4 shadow-lg">
      ...
      <p className={`font-black ${small ? "text-lg" : "text-3xl"} ${accent ?? "text-gray-700"}`}>{value}</p>
    </div>
  )
}
