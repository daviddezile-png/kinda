...
export default async function AdminPage() {
  ...
  const schoolOptions = schools.map((school: { id: string; name: string }) => ({
    id: school.id,
    name: school.name,
  }));
      ...
      <div className="relative z-10 mx-auto max-w-4xl space-y-6">
        <header className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-gray-400">
              {session.user.name} · {schools.length} schools · {teachers.length}{" "}
              teachers
            </p>
          </div>
          <LogoutButton />
        ...
        <section className={card}>
          ...
          <ul className="mt-4 divide-y divide-gray-100">
            {schools.length === 0 && (
              <li className="py-4 text-sm text-gray-400">No schools yet.</li>
            )}
            {schools.map(
              (school: {
                id: string;
        ...
        <section className={card}>
          ...
          <ul className="mt-4 divide-y divide-gray-100">
            {teachers.length === 0 && (
              <li className="py-4 text-sm text-gray-400">No teachers yet.</li>
            )}
            {teachers.map(
              (teacher: {
                id: string;
                ...
                >
                  <div className="min-w-0">
                    <p className="truncate font-bold text-gray-700">
                      {teacher.name}
                      {!teacher.isActive && (
                        <span className="ml-2 text-xs font-bold text-rose-500">
                          suspended
                        </span>
                      )}
                    </p>
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    <TeacherPasswordReset userId={teacher.id} />
                    <form
                    ...
                    >
                      ...
                      >
                        {teacher.isActive ? "Suspend" : "Activate"}
                      </button>
                    </form>
                  </div>
                </li>
